...
class application:
    ...
    def setup_gui(self,s):
        ...
        Button(newFrame,text='kernel sharp',bd=2,fg='white',bg='blue',font=('',15)
            ,command=self.SharpKernel).pack(side=LEFT)       
        Button(newFrame,text='kernel Edge',bd=2,fg='white',bg='blue',font=('',15)
    ...
    def valueArray(self):
        ...
        for i  in range(width):
            for j in range(height):
                print("image value R : "+str(image[i,j,0]))
                print("image value G : "+str(image[i,j,1]))
                print("image value B : "+str(image[i,j,2]))
